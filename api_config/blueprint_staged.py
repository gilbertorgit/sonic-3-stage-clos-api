...
def blueprint_device_profile_3_stage(blueprint_name, spine_device, leaf_device, borderleaf_device):
    ...
    for id in device_id_list:
        if id != None:
            ...
            elif id['label'] == 'sonic_border_leaf_001_leaf1':
        ...
        if device != None:
            if 'spine1' in value['label'] and value.get('system_type') == 'switch':
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.215':
                ...
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.216':
                        response = blueprint_physical_device(blueprint_name, sysDevice['device_key'], value['id'])

            elif 'sonic_border_leaf_001_leaf1' in value['label'] and value.get('system_type') == 'switch':
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.220':
                        response = blueprint_physical_device(blueprint_name, sysDevice['device_key'], value['id'])

            elif 'sonic_mclag_leaf_001_leaf1' in value['label'] and value.get('system_type') == 'switch':
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.217':
                        response = blueprint_physical_device(blueprint_name, sysDevice['device_key'], value['id'])

            elif 'sonic_mclag_leaf_001_leaf2' in value['label'] and value.get('system_type') == 'switch':
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.218':
                        response = blueprint_physical_device(blueprint_name, sysDevice['device_key'], value['id'])

            elif 'sonic_single_leaf_001_leaf1' in value['label'] and value.get('system_type') == 'switch':
                for sysDevice in devices_response.json()['items']:
                    if sysDevice['facts']['mgmt_ipaddr'] == '192.168.0.219':
                        response = blueprint_physical_device(blueprint_name, sysDevice['device_key'], value['id'])
...
def set_blueprint_svi_subnet_mclag(blueprint_name, ip_pool,):

    print(f"--------------------Setting SVI Subnets - MLAG Domain : {blueprint_name} loopback pool: {ip_pool}")

    data = f'''
        {{"pool_ids":["{ip_pool}"]}}
    '''

    url = f'{url_ba.apstra_url}{url_ba.blueprints_url}/{blueprint_name}{url_ba.blueprints_resource_groups_svi_subnet_mlag}'

    response = apstra_put(url=url, data=data)
    return response


def set_blueprint_vtep_ip(blueprint_name, ip_pool,):
    ...
    url = f'{url_ba.apstra_url}{url_ba.blueprints_url}/{blueprint_name}{url_ba.blueprints_resource_groups_vtep_ips}'
...
def set_deploy_blueprint(blueprint_name, description):
    ...
    print(f"--------------------Committing blueprint: {blueprint_name}")
